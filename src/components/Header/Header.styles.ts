...
const HeaderMobileBar = styled.div<{ index: number; count: number; open: boolean; color: string }>`
  ...
  transition: 0.4s ${(props) => (props.count - props.index) * 0.1}s;
  ...
  ${(props) =>
    props.open &&
    css`
      transition: 0.4s ${props.index * 0.1}s;
      transform: translateY(0%);
    `}
`;

export { HeaderWrapper, HeaderInner, HeaderLogo, HeaderTitle, HeaderLinks, HeaderLinksRow, HeaderLink, HeaderBurger, HeaderMobile, HeaderMobileBar };
